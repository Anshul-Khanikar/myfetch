...
static void cleanup(void) {
  ...
  if (cleaned) return; // Prevent double-firing
  cleaned = 1;

  if (termios_saved)
    tcsetattr(STDIN_FILENO, TCSANOW, &orig_termios);

  // Destroy alternate screen buffer and show cursor
...
static int load_logo_file(void) {
  ...
  if (!home)
    return 0;
  snprintf(path, sizeof(path), "%s/.config/myfetch/logo.txt", home);
...
static char label_color[16] = "32"; // changed to green
...
static void load_config(void) {
  ...
  if (!home)
    return;
  char path[512];
  snprintf(path, sizeof(path), "%s/.config/myfetch/config", home);
  FILE *fp = fopen(path, "r");
  if (!fp)
    return;
...
static void gather_wm(void) {
  ...
  // 2. Try process list by scanning 
 if (!wm[0]) {
...
if (show_info) {
    FILE *fp = fopen("/tmp/myfetch_stats.txt", "r");
    if (fp) {
      char line_buf[MAX_LINE_LEN];
      while (fgets(line_buf, sizeof(line_buf), fp) && fetch_line_count < MAX_FETCH_LINES) {
        int len = strlen(line_buf);
        while (len > 0 && (line_buf[len - 1] == '\n' || line_buf[len - 1] == '\r')) {
          line_buf[--len] = '\0';
        }

        // Map the exact row positions of the updating fields
        if (strstr(line_buf, "Uptime")) field_line[F_UPTIME] = fetch_line_count;
        if (strstr(line_buf, "Memory")) field_line[F_MEMORY] = fetch_line_count;
